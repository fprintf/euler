...
int max(const int * row, ptrdiff_t col)
{
    return row[col + (row[col] < row[col+1])];
}
...
#define asizeof(a) (sizeof (a) / sizeof *(a))
    for (row = asizeof(array)-2; row > -1; --row) 
        for (col = 0; col < row+1; ++col) 
            array[row][col] += max(array[row+1], col);
    printf("Found sum of: %d\n", array[0][0]);
    return 0;
}
